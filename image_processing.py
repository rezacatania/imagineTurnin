...
def intensifybound(intensity, quantity):
    if (intensity + quantity) >= 0 and (intensity + quantity <= 255):
...
def intensifypixel(pixel, quantity):
    return((intensifybound(pixel[0], quantity)),
           (intensifybound(pixel[1], quantity)),
           (intensifybound(pixel[2], quantity)))
...
def bi(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x, y] = intensifypixel(pixels3d[x, y], 10)
